//@author- Anudish Jain
	...
	contract Rent is Owned {
					...
					else
					{
						...
						if(home.completed == false)
						{
							...
							if((home.monthDuration < 12)&&(home.monthDuration > 0))
							{
								...
								registerHome("Property's Information successfully entered, Proceed to Step-3", (home.registerFee / 8380));
							}
		...
		function feePayment(string _sign, uint _currentRate) external payable {
			...
			require((msg.value * _currentRate) == registerFeeMap[msg.sender]);
		...
		function tenantAccept(string _sign, uint _currentRate) external payable {
			...
			require(msg.value == securityFeeMap[msg.sender]  * 10**18/ _currentRate);
